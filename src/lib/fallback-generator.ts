...
export function generateFallbackStory(
  ...
  for (const paragraph of buildExpansionParagraphs(cast, primaryRule, secondaryRule)) {
...
function buildExpansionParagraphs(cast: string[], primaryRule: string, secondaryRule: string): string[] {
  const lead = cast[0];

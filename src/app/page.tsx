        ...
        <>
          <div className="mt-4 grid min-w-0 gap-4 md:grid-cols-3">
            {state.openings.map((opening, index) => (
              <button aria-pressed={state.selectedIndex === index} className={`block min-h-36 w-full min-w-0 whitespace-normal rounded-lg border p-4 text-left transition ${state.selectedIndex === index ? "border-lantern-gold bg-lantern-gold/15 shadow-soft outline outline-2 outline-lantern-gold" : "border-paper/12 bg-night-ink/35 hover:border-lantern-gold/45"}`} key={`${opening.title}-${index}`} onClick={() => onSelect(index)} type="button">
                <span className="block text-xs font-semibold uppercase tracking-[0.12em] text-lantern-gold">{opening.toneLabel || "Story opening"}</span>
                <span className="mt-2 block text-lg font-semibold leading-tight text-paper">{opening.title || `Opening ${index + 1}`}</span>
                <span className="mt-3 block whitespace-pre-line text-sm leading-6 text-paper/75">{opening.openingText}</span>
              </button>
            ))}
          </div>
...
function normalizeFirstPageOpenings(value: unknown): FirstPageOpening[] { const items = Array.isArray(value) ? value : []; return items.filter(Boolean).slice(0, 3).map((item, index) => { const candidate = item as Partial<FirstPageOpening> & { hook?: unknown; tone?: unknown; text?: unknown; prose?: unknown }; const openingText = [candidate.openingText, candidate.text, candidate.prose].find((field) => typeof field === "string" && field.trim()); return { title: typeof candidate.title === "string" && candidate.title.trim() ? candidate.title.trim() : typeof candidate.hook === "string" && candidate.hook.trim() ? candidate.hook.trim() : `Opening ${index + 1}`, toneLabel: typeof candidate.toneLabel === "string" && candidate.toneLabel.trim() ? candidate.toneLabel.trim() : typeof candidate.tone === "string" && candidate.tone.trim() ? candidate.tone.trim() : "Story opening", openingText: typeof openingText === "string" ? openingText.trim() : "" }; }).filter((opening) => opening.openingText.trim().length > 0); }

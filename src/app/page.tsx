...
const DEFAULT_BUILD_INFO: BuildInfo = { appVersion: "0.7.1", buildEnvironment: "development", gitBranch: "local", commitSha: "unknown", shortCommitSha: "unknown", buildTimestamp: "unknown", vercelUrl: "local" };
...
type UploadControlsProps = { artifactType: InputArtifactType; libraryArtifacts: InputArtifact[]; onChange: (value: UploadState) => void; onRemoveFromLibrary: (type: InputArtifactType, artifactId?: string) => void; onSaveToLibrary: (type: InputArtifactType, value: UploadState) => void; onSelectFromLibrary: (type: InputArtifactType, artifactId: string) => void; uploadTitle: string; value: UploadState };
...
export default function Home() {
  ...
  async function handleGenerate() {
    ...
    try {
      const response = await fetch("/api/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ worldBible: worldBible.content, characterProfiles: characterProfiles.content, storySeed: storySeed.content, storyRules: storyRules.content, genrePreset, narrativeArchitecture, characterArc, endingType, lengthTarget }) });
      ...
      setStoryResponse({ ...normalizedResponse, metadata: { ...normalizedResponse.metadata, generationStartedAt: startedAt.toISOString(), generationFinishedAt: finishedAt.toISOString(), generationDurationSeconds: Math.max(0, Math.round((finishedAt.getTime() - startedAt.getTime()) / 1000)) } });
    } catch (caughtError) {
  ...
  function handleApplyWorldTemplate(template: WorldTemplate, mode: "add" | "replace") {
    const card = formatWorldTemplateBible(template);
    const currentWorld = worldBible.content.trim();
    setWorldBible({ name: mode === "replace" || !worldBible.name ? `${template.id}.md` : worldBible.name, content: mode === "replace" || !currentWorld ? card : `${currentWorld}\n\n${card}` });
    setRecommendation(null);
    setStoryResponse(null);
    setStatusMessage(mode === "replace" ? `${template.title} is now the active Storyworld.` : `${template.title} added to Storyworld.`);
  }
  ...
  function handleApplyCharacterArchetype(preset: CharacterArchetypePreset, mode: "add" | "replace") {
    ...
    setCharacterProfiles({ name: characterProfiles.name || "cast-archetypes.md", content: mode === "replace" || !currentProfiles ? card : `${currentProfiles}\n\n${card}` });
    setRecommendation(null);
    setStoryResponse(null);
    setStatusMessage(mode === "replace" ? `${preset.name} replaced the current Cast input.` : `${preset.name} added to Cast.`);
  }
  ...
  function handleRemoveInputArtifact(type: InputArtifactType, artifactId?: string) {
    ...
    persistInputArtifacts(nextArtifacts);
    setInputArtifacts(nextArtifacts);
    if (artifactId === worldBible.libraryArtifactId) setWorldBible({ name: worldBible.name, content: worldBible.content });
    if (artifactId === characterProfiles.libraryArtifactId) setCharacterProfiles({ name: characterProfiles.name, content: characterProfiles.content });
    if (artifactId === storySeed.libraryArtifactId) setStorySeed({ name: storySeed.name, content: storySeed.content });
    if (artifactId === storyRules.libraryArtifactId) setStoryRules({ name: storyRules.name, content: storyRules.content });
      ...
      <section className="mx-auto flex w-full max-w-7xl flex-col gap-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,420px)_1fr]">
          <section className="flex flex-col gap-4">
            ...
            <StoryworldSection artifactType="worldBible" disabled={isGenerating} libraryArtifacts={inputArtifacts.filter((artifact) => artifact.type === "worldBible")} onApplyTemplate={handleApplyWorldTemplate} onChange={setWorldBible} onRemoveFromLibrary={handleRemoveInputArtifact} onSaveToLibrary={handleSaveInputArtifact} onSelectFromLibrary={handleSelectInputArtifact} value={worldBible} />
            <CastSection artifactType="characterProfiles" disabled={isGenerating} libraryArtifacts={inputArtifacts.filter((artifact) => artifact.type === "characterProfiles")} onApplyArchetype={handleApplyCharacterArchetype} onChange={setCharacterProfiles} onRemoveFromLibrary={handleRemoveInputArtifact} onSaveToLibrary={handleSaveInputArtifact} onSelectFromLibrary={handleSelectInputArtifact} value={characterProfiles} />
            <UploadPanel artifactType="storySeed" description="The image, event, conflict, or question that starts the story." libraryArtifacts={inputArtifacts.filter((artifact) => artifact.type === "storySeed")} onChange={setStorySeed} onRemoveFromLibrary={handleRemoveInputArtifact} onSaveToLibrary={handleSaveInputArtifact} onSelectFromLibrary={handleSelectInputArtifact} title="Story Spark" uploadTitle="Upload your own Story Spark" value={storySeed} />
            <UploadPanel artifactType="storyRules" description="Default craft rules are used automatically. Advanced users can customize them." libraryArtifacts={inputArtifacts.filter((artifact) => artifact.type === "storyRules")} onChange={setStoryRules} onRemoveFromLibrary={handleRemoveInputArtifact} onSaveToLibrary={handleSaveInputArtifact} onSelectFromLibrary={handleSelectInputArtifact} title="Craft Rules" uploadTitle="Upload your own Craft Rules" value={storyRules} />
...
function StoryworldSection(props: Omit<UploadControlsProps, "uploadTitle"> & { disabled: boolean; onApplyTemplate: (template: WorldTemplate, mode: "add" | "replace") => void }) {
  const { disabled, onApplyTemplate, ...uploadProps } = props;
  return <section className="rounded-md border border-ink/10 bg-white/70 p-4 shadow-soft"><div className="flex flex-col gap-1"><h2 className="text-lg font-semibold text-ink">Storyworld</h2><p className="text-sm leading-6 text-ink/65">Choose a world, add your own, or combine both.</p></div><WorldTemplatePicker disabled={disabled} onApply={onApplyTemplate} /><UploadControls {...uploadProps} uploadTitle="Upload your own Storyworld" /></section>;
}

function CastSection(props: Omit<UploadControlsProps, "uploadTitle"> & { disabled: boolean; onApplyArchetype: (preset: CharacterArchetypePreset, mode: "add" | "replace") => void }) {
  const { disabled, onApplyArchetype, ...uploadProps } = props;
  return <section className="rounded-md border border-ink/10 bg-white/70 p-4 shadow-soft"><div className="flex flex-col gap-1"><h2 className="text-lg font-semibold text-ink">Cast</h2><p className="text-sm leading-6 text-ink/65">Choose a character, antihero, witness, keeper, singer, repairer, or upload your own.</p></div><CharacterArchetypePicker disabled={disabled} onApply={onApplyArchetype} /><UploadControls {...uploadProps} uploadTitle="Upload your own Cast" /></section>;
}

function WorldTemplatePicker({ disabled, onApply }: { disabled: boolean; onApply: (template: WorldTemplate, mode: "add" | "replace") => void }) {
  const [selectedId, setSelectedId] = useState("");
  const [showDetails, setShowDetails] = useState(false);
  const selectedTemplate = useMemo(() => WORLD_TEMPLATES.find((template) => template.id === selectedId) ?? null, [selectedId]);
  return <div className="mt-4 rounded-md border border-ink/10 bg-paper/80 p-3"><label className="flex flex-col gap-2"><span className="text-sm font-semibold text-ink">Choose a world template</span><select className="rounded-md border border-ink/15 bg-white px-3 py-2 text-sm text-ink outline-none transition focus:border-brass focus:ring-2 focus:ring-brass/20" onChange={(event) => { setSelectedId(event.target.value); setShowDetails(false); }} value={selectedId}><option value="">Select a world template</option>{WORLD_TEMPLATES.map((template) => <option key={template.id} value={template.id}>{template.title}</option>)}</select></label>{selectedTemplate ? <SelectedWorldPanel disabled={disabled} onApply={onApply} onToggleDetails={() => setShowDetails((value) => !value)} showDetails={showDetails} template={selectedTemplate} /> : <p className="mt-4 rounded-md bg-paper/80 px-3 py-2 text-sm text-ink/60">Select a world to preview its core rule.</p>}</div>;
}
...
function SelectedWorldPanel({ disabled, onApply, onToggleDetails, showDetails, template }: { disabled: boolean; onApply: (template: WorldTemplate, mode: "add" | "replace") => void; onToggleDetails: () => void; showDetails: boolean; template: WorldTemplate }) {
  return <article className="mt-4 rounded-md border border-ink/10 bg-paper/80 p-3"><h3 className="text-base font-semibold leading-6 text-ink">{template.title}</h3><p className="mt-2 text-sm leading-6 text-ink/70">{template.shortDescription}</p><dl className="mt-3 rounded-md bg-white/65 px-3 py-2"><dt className="text-xs font-semibold uppercase tracking-[0.14em] text-ink/45">Core Rule</dt><dd className="mt-1 text-sm leading-6 text-ink/75">{template.coreRule}</dd></dl><div className="mt-3 flex flex-wrap gap-2"><button className="rounded-md bg-ink px-3 py-2 text-xs font-semibold text-paper transition hover:bg-ink/90 disabled:cursor-not-allowed disabled:opacity-50" disabled={disabled} onClick={() => onApply(template, "add")} type="button">Add to Storyworld</button><button className="rounded-md border border-brass/40 bg-white/75 px-3 py-2 text-xs font-semibold text-brass transition hover:border-brass hover:bg-paper disabled:cursor-not-allowed disabled:opacity-50" disabled={disabled} onClick={() => onApply(template, "replace")} type="button">Use this Storyworld</button><button className="rounded-md border border-ink/15 bg-white/75 px-3 py-2 text-xs font-semibold text-ink transition hover:bg-paper" onClick={onToggleDetails} type="button">{showDetails ? "Hide Details" : "View Details"}</button></div>{showDetails ? <dl className="mt-4 grid gap-2 text-sm text-ink/75"><DetailItem label="Full Text" value={template.fullWorldBibleText} /><DetailItem label="Best Characters" value={template.bestCharacters} /><DetailItem label="Story Pressure" value={template.storyPressure} /><DetailItem label="Sensory Palette" value={template.sensoryPalette} /></dl> : null}</article>;
}

function CharacterArchetypePicker({ disabled, onApply }: { disabled: boolean; onApply: (preset: CharacterArchetypePreset, mode: "add" | "replace") => void }) {
  const [selectedName, setSelectedName] = useState("");
  const [showDetails, setShowDetails] = useState(false);
  const selectedPreset = useMemo(() => CHARACTER_ARCHETYPE_PRESETS.find((preset) => preset.name === selectedName) ?? null, [selectedName]);
  return <div className="mt-4 rounded-md border border-ink/10 bg-paper/80 p-3"><label className="flex flex-col gap-2"><span className="text-sm font-semibold text-ink">Choose a character archetype</span><select className="rounded-md border border-ink/15 bg-white px-3 py-2 text-sm text-ink outline-none transition focus:border-brass focus:ring-2 focus:ring-brass/20" onChange={(event) => { setSelectedName(event.target.value); setShowDetails(false); }} value={selectedName}><option value="">Select an archetype</option>{CHARACTER_ARCHETYPE_PRESETS.map((preset) => <option key={preset.name} value={preset.name}>{formatArchetypeOption(preset)}</option>)}</select></label>{selectedPreset ? <SelectedCharacterPanel disabled={disabled} onApply={onApply} onToggleDetails={() => setShowDetails((value) => !value)} preset={selectedPreset} showDetails={showDetails} /> : <p className="mt-4 rounded-md bg-paper/80 px-3 py-2 text-sm text-ink/60">Select a character to preview the compact card.</p>}</div>;
}
...
function SelectedCharacterPanel({ disabled, onApply, onToggleDetails, preset, showDetails }: { disabled: boolean; onApply: (preset: CharacterArchetypePreset, mode: "add" | "replace") => void; onToggleDetails: () => void; preset: CharacterArchetypePreset; showDetails: boolean }) {
  return <article className="mt-4 rounded-md border border-ink/10 bg-paper/80 p-3"><div className="grid gap-3 sm:grid-cols-[84px_1fr]"><PencilPortrait name={preset.name} /><div className="min-w-0"><h3 className="text-base font-semibold leading-6 text-ink">{preset.name}</h3><p className="mt-1 text-sm font-semibold text-brass">{preset.archetype}</p><p className="mt-2 text-sm leading-6 text-ink/70">{preset.function}</p></div></div><div className="mt-3 flex flex-wrap gap-2"><button className="rounded-md bg-ink px-3 py-2 text-xs font-semibold text-paper transition hover:bg-ink/90 disabled:cursor-not-allowed disabled:opacity-50" disabled={disabled} onClick={() => onApply(preset, "add")} type="button">Add to Cast</button><button className="rounded-md border border-brass/40 bg-white/75 px-3 py-2 text-xs font-semibold text-brass transition hover:border-brass hover:bg-paper disabled:cursor-not-allowed disabled:opacity-50" disabled={disabled} onClick={() => onApply(preset, "replace")} type="button">Replace Cast</button><button className="rounded-md border border-ink/15 bg-white/75 px-3 py-2 text-xs font-semibold text-ink transition hover:bg-paper" onClick={onToggleDetails} type="button">{showDetails ? "Hide Details" : "View Details"}</button></div>{showDetails ? <dl className="mt-4 grid gap-2 text-sm text-ink/75"><DetailItem label="Enneagram" value={preset.enneagram} /><DetailItem label="Core Desire" value={preset.coreDesire} /><DetailItem label="Core Fear" value={preset.coreFear} /><DetailItem label="Backstory" value={preset.backstory} /><DetailItem label="Conflict Engine" value={preset.conflictEngine} /></dl> : null}</article>;
}

function UploadPanel({ artifactType, description, libraryArtifacts, onChange, onRemoveFromLibrary, onSaveToLibrary, onSelectFromLibrary, title, uploadTitle, value }: UploadControlsProps & { description: string; title: string }) {
  return <section className="rounded-md border border-ink/10 bg-white/70 p-4 shadow-soft"><div className="flex flex-col gap-1"><h2 className="text-lg font-semibold text-ink">{title}</h2><p className="text-sm leading-6 text-ink/65">{description}</p></div><UploadControls artifactType={artifactType} libraryArtifacts={libraryArtifacts} onChange={onChange} onRemoveFromLibrary={onRemoveFromLibrary} onSaveToLibrary={onSaveToLibrary} onSelectFromLibrary={onSelectFromLibrary} uploadTitle={uploadTitle} value={value} /></section>;
}

function UploadControls({ artifactType, libraryArtifacts, onChange, onRemoveFromLibrary, onSaveToLibrary, onSelectFromLibrary, uploadTitle, value }: UploadControlsProps) {
...
function SelectControl({ label, value, options, onChange }: { label: string; value: string; options: readonly string[] | readonly { value: string; label: string }[]; onChange: (value: string) => void }) {

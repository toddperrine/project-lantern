...
function getStoryTypeCompatibility(story: StoryStart, mood: Mood): { compatible: boolean; result: string } {
  ...
  return getStoryTypeTextCompatibility(chip, [story.mood, story.genre, story.title, story.premise, story.seed, story.world, story.rules, ...story.tags].join(" "));
}
...
export default function Home() {
  ...
  const [readerScrollDiagnostics, setReaderScrollDiagnostics] = useState<ReaderScrollDiagnostics>({ nextEpisodeClicked: "no", continuationLoaded: "no", scrollResetAttempted: "no", scrollTargetUsed: "none" });
  ...
  const [storyTypeSelectionDiagnostics, setStoryTypeSelectionDiagnostics] = useState<StoryTypeSelectionDiagnostics>({ selectedStoryTypeChipId: AVAILABLE_MOOD_CHIPS[0], selectedStoryTypeChipLabel: getStoryTypeChip(AVAILABLE_MOOD_CHIPS[0]).label, selectedChipId: AVAILABLE_MOOD_CHIPS[0], selectedChip: getStoryTypeChip(AVAILABLE_MOOD_CHIPS[0]).label, availableChips: formatMoodChipList(AVAILABLE_MOOD_CHIPS), storySparkUsed: "no", selectedStorySparkId: "none", selectedStorySparkTitle: "none", selectedStorySparkMatchedChip: "none", directChipGuidanceUsed: "no", compatibilityResult: "not evaluated", chipCompatibilityResult: "not evaluated", fallbackSelectionUsed: "no", selectedChipPreservedDuringGeneration: "not generating", storyTypeSelectionMode: "selected", storySeedSource: "not generated", visibleCategoryLabel: getStoryTypeChip(AVAILABLE_MOOD_CHIPS[0]).label });
  ...
  const shouldShowFirstRunStoryFitPrompt = hasLoadedReaderProfileState && !hasReaderProfilePreferences(readerProfile.explicitReaderPreferences) && !storyFitOnboardingDismissed;
  ...
  async function loadAuthenticatedReaderProfile() {
    ...
    try {
      ...
      if (response.status === 503) {
        updateCloudReaderProfileSync({ profileId, status: "unavailable", lastLoadStatus: "unavailable", lastSaveOutcome: "unavailable", lastError: payload.error ?? "Reader profile cloud persistence is unavailable.", localProfileExists: readerProfileExistsInLocalStorage(), ...cloudDiagnosticUpdate(payload.diagnostic) });
        return;
      }
      if (!response.ok) {
  ...
  async function handleGenerate(overrides: { generationMode: GenerationMode; worldBible?: string; characterProfiles?: string; storySeed?: string; storyRules?: string; genrePreset?: GenrePreset; selectedStoryTypeChip?: StoryTypeChip; storySeedSource?: string; narrativeArchitecture?: NarrativeArchitecture; characterArc?: CharacterArc; endingType?: EndingType; lengthTarget?: LengthTarget; readerMood?: ReaderMoodSnapshot | null; presentation?: Exclude<GeneratedStoryPresentation, null>; loadingMessage?: string; signalSource?: ReaderProfileEventSource; generationSource?: Exclude<GenerationSource, null>; continuationStoryId?: string; continuationContextIncluded?: boolean; selectedSeriesId?: string | null; sourceStoryId?: string | null; continuationDiagnostics?: ContinuationGenerationDiagnostics }) {
    ...
    const activeReaderProfile = authState.currentUser ? readerProfile : readReaderProfile();
    const activeCanonicalProfile = authState.currentUser
      ? canonicalReaderProfileFromReaderProfile(activeReaderProfile, authState.currentUser.id, "cloud")
      : mirrorCanonicalReaderProfilePreferences(loadCanonicalReaderProfile(), activeReaderProfile, eerieReaderProfile);
    ...
    try {
      ...
      if (nextGenerationSource === "continue-story" || nextPresentation === "continuation") {
        setReaderScrollDiagnostics((current) => ({ ...current, continuationLoaded: "yes", scrollResetAttempted: "pending", scrollTargetUsed: "pending" }));
      }
      const generatedAt = new Date().toISOString();
      const canonicalAfterGenerationDraft = { ...activeCanonicalProfile, updatedAt: generatedAt, signals: { ...activeCanonicalProfile.signals, lastStoryGeneratedAt: generatedAt, lastGenerationUsedCanonicalProfile: true } };
      const canonicalAfterGeneration = authState.currentUser ? canonicalAfterGenerationDraft : saveCanonicalReaderProfile(canonicalAfterGenerationDraft);
  ...
  function applyStoryStart(story: StoryStart, options: { updateActiveMood?: boolean } = {}) {
  ...
  async function saveReaderProfileToCloud(profileId: string, profile: ReaderProfile) {
    ...
    if (response.status === 503) {
      updateCloudReaderProfileSync({ profileId, status: "unavailable", lastSaveOutcome: "unavailable", lastError: payload.error ?? "Reader profile cloud persistence is unavailable.", localUpdatedAt: profile.updatedAt, localProfileExists: readerProfileExistsInLocalStorage(), ...cloudDiagnosticUpdate(payload.diagnostic) });
      return;
    }

    if (!response.ok) {
      updateCloudReaderProfileSync({ profileId, status: "error", lastSaveOutcome: "error", lastError: payload.error ?? "Reader profile cloud save failed.", localUpdatedAt: profile.updatedAt, localProfileExists: readerProfileExistsInLocalStorage(), ...cloudDiagnosticUpdate(payload.diagnostic) });
  ...
  function cloudDiagnosticUpdate(diagnostic: ReaderProfileApiDiagnostic | undefined): Partial<CloudReaderProfileSyncState> {
    ...
    return {
      ...
      initializedDefaultCloudProfile: Boolean(diagnostic.initializedDefaultCloudProfile),
      lastSanitizedErrorName: diagnostic.sanitizedErrorName ? String(diagnostic.sanitizedErrorName) : "none",
      lastSanitizedErrorCode: diagnostic.sanitizedErrorCode === null || diagnostic.sanitizedErrorCode === undefined ? "none" : String(diagnostic.sanitizedErrorCode)
    };
  }
  ...
  function handleContinueSavedStoryById(storyId: string) {
    ...
    if (!story) {
      setStatusMessage("Could not continue that saved episode because it was not found.");
      return;
    }
  ...
  function handleSavedEpisodeNext() {
    ...
    if (nextSavedEpisode) {
      setReaderScrollDiagnostics((current) => ({ ...current, nextEpisodeClicked: "yes", continuationLoaded: "yes", scrollResetAttempted: "pending", scrollTargetUsed: "pending" }));
  ...
  function handleStoryFeedbackChange(story: LibraryStory, rating: StoryFeedbackRating, reasons: StoryFeedbackReason[]) {
    ...
    const currentCanonicalProfile = authState.currentUser ? canonicalReaderProfileFromReaderProfile(readerProfile, authState.currentUser.id, "cloud") : loadCanonicalReaderProfile();
    ...
    const nextCanonicalProfile = authState.currentUser ? { ...nextCanonicalProfileDraft, source: "cloud" as const } : saveCanonicalReaderProfile(nextCanonicalProfileDraft);
  ...
  function handleSkipStoryFitOnboarding() {
    ...
    setStatusMessage("Story fit setup skipped. You can start it later from Account / Profile.");
  }
  ...
  return (
    <main className="min-h-screen overflow-x-hidden px-3 pb-24 pt-3 text-paper sm:px-4 md:px-8 md:py-7">
      <section className="mx-auto flex w-full max-w-7xl min-w-0 flex-col gap-5 md:gap-6">
        ...
        />
        {activeView === "home" && !(currentGeneratedStory && generatedStoryPresentation) ? (
          <section
            className="grid min-w-0 gap-3 rounded-xl border border-paper/10 bg-night-ink/90 p-4 md:hidden"
            data-mobile-home-actions="true"
          >
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-lantern-gold">
              Home
            </p>
            <h1 className="text-2xl font-semibold leading-tight text-paper">
        ...
        {isStoryFitOnboardingOpen ? <StoryFitOnboardingPanel initialPreferences={readerProfile.explicitReaderPreferences} onCancel={() => setIsStoryFitOnboardingOpen(false)} onSave={handleSaveStoryFitOnboarding} /> : null}

        <div className="hidden min-w-0 gap-3 md:grid">{diagnosticsPanels}</div>
    ...
    <section className="grid gap-3 rounded-xl border border-lantern-gold/20 bg-paper/5 p-4 text-sm text-paper/75 md:max-w-xl" aria-label="Reader account">
      ...
      {!auth.authConfigured ? (
        ...
        <form className="grid gap-3" onSubmit={handleSignIn}>
          ...
          <button className="min-h-12 rounded-xl bg-lantern-gold px-4 py-3 font-semibold text-night-ink disabled:cursor-not-allowed disabled:opacity-60" disabled={isSubmitting || !password.trim()} type="submit">{isSubmitting ? "Signing in…" : "Sign in"}</button>
          {!password.trim() ? <p className="text-xs text-paper/55">Enter your password.</p> : null}
      ...
      {auth.successMessage ? <Status tone="success">{auth.successMessage}</Status> : null}
      {auth.errorMessage ? <Status tone="error">{auth.errorMessage}</Status> : null}
    </section>
  );
}
  ...
  return (
    <details className="min-w-0 rounded-md border border-paper/10 bg-paper/5 p-3 text-xs text-paper/65">
      ...
      <div className="mt-3 grid gap-1 sm:grid-cols-2">
        ...
        <p><span className="font-semibold text-paper/80">Last Cognito error code:</span> {lastCognitoErrorCode || "none"}</p>
        <p><span className="font-semibold text-paper/80">Reset flow state:</span> {resetFlowState}</p>
...
function EpisodeReader({ continueLabel = "Continue this story", episodeNumber, eyebrow, feedback, generationBlockedBecauseUnsavedFeedback, generationProfileSnapshot, isGenerating, onContinue, onExport, onFeedbackChange, onFeedbackDraftStateChange, onScrollResetDiagnostics, onStartDifferent, story }: { continueLabel?: string; episodeNumber?: number | null; eyebrow: string; feedback: StoryFeedbackSignal | null; generationBlockedBecauseUnsavedFeedback: boolean; generationProfileSnapshot?: ReaderProfileGenerationSnapshot; isGenerating: boolean; onContinue: () => void; onExport: () => void; onFeedbackChange: (story: LibraryStory, rating: StoryFeedbackRating, reasons: StoryFeedbackReason[]) => void; onFeedbackDraftStateChange: (state: { hasUnsavedChanges: boolean; saveBlockedBecauseRatingMissing: boolean }) => void; onScrollResetDiagnostics: (diagnostics: ReaderScrollDiagnostics | ((current: ReaderScrollDiagnostics) => ReaderScrollDiagnostics)) => void; onStartDifferent: () => void; story: LibraryStory }) {
  ...
  useEffect(() => {
    ...
    animationFrame = window.requestAnimationFrame(() => {
      ...
      const readerContainer = readerTopRef.current?.closest<HTMLElement>("[data-reader-scroll-container=\"true\"]");
      if (readerContainer) {
      ...
      targets.push("window");
      onScrollResetDiagnostics((current) => ({ ...current, continuationLoaded: current.continuationLoaded === "pending" ? "yes" : current.continuationLoaded, scrollResetAttempted: "yes", scrollTargetUsed: targets.join("+") || "none" }));
    });
  ...
  return (
    <article className="grid min-w-0 gap-5 rounded-md border border-lantern-gold/25 bg-paper/10 p-4 shadow-soft sm:p-6" data-reader-scroll-container="true">
...
function HomePrimaryActions({
  ...
  return (
    <section className="grid min-w-0 max-w-full gap-3 rounded-xl border border-paper/10 bg-paper/10 p-4 md:p-5">
      ...
      <h1 className="break-words text-2xl font-semibold leading-tight text-paper md:text-3xl">What do you want to do?</h1>
...
function HomeView(props: { showStoryFitPrompt?: boolean; onStartStoryFitSetup?: () => void; onSkipStoryFitSetup?: () => void; activeMood: Mood; canUseDemoStory: boolean; continueDirection: string; hasDemoStory: boolean; isDirectionOpen: boolean; isGenerating: boolean; isContinuationGenerating: boolean; isNewStoryGenerating: boolean; latestStory: LibraryStory | null; onClearDemoStory: () => void; onContinue: (direction?: string) => void; onDirectionChange: (value: string) => void; onExportStory: () => void; onLoadDemoStory: () => void; onMoodSelect: (mood: Mood) => void; onPassReadyStory: (item: ReadyStoryQueueItem) => void; onReadReadyStory: (item: ReadyStoryQueueItem) => void; onSaveReadyStoryForLater: (item: ReadyStoryQueueItem) => void; onOpenCreate: () => void; onOpenLibrary: () => void; onStartNewStory: () => void; onStartRecommendation: (story: StoryStart) => void; onToggleDirection: () => void; readyStoryQueue: ReadyStoryQueueItem[]; savedForLaterStoryQueue: ReadyStoryQueueItem[]; showStoryStartOptions: boolean; suggestedStarts: StoryStart[] }) {
  ...
  return (
    <div className="grid min-w-0 max-w-full gap-6 overflow-x-hidden md:gap-8">
      <HomePrimaryActions
        hasLatestStory={Boolean(latestStory)}
      ...
      />
      {showStoryFitPrompt && onStartStoryFitSetup && onSkipStoryFitSetup ? <StoryFitFirstRunCard onSkip={onSkipStoryFitSetup} onStart={onStartStoryFitSetup} /> : null}
      {latestStory && storyBrief ? <div className="md:hidden"><MobileCurrentStoryCard brief={storyBrief} isGenerating={isContinuationGenerating} isRecapOpen={isRecapOpen} onCloseRecap={() => setIsRecapOpen(false)} onContinue={onContinue} onOpenRecap={() => setIsRecapOpen(true)} story={latestStory} /></div> : null}
...
function StoryFitFirstRunCard({ onSkip, onStart }: { onSkip: () => void; onStart: () => void }) {
  return (
...
function StoryFitOnboardingPanel({ initialPreferences, onCancel, onSave }: { initialPreferences: ReaderProfile["explicitReaderPreferences"]; onCancel: () => void; onSave: (preferences: ReaderProfile["explicitReaderPreferences"]) => void }) {
  ...
  const countVisibleSelections = (values: string[], options: { label: string }[]) => values.filter((value) => options.some((option) => option.label === value)).length;
  const avoidanceDraftValue = avoidanceDraft.trim().replace(/\s+/g, " ");
  const avoidanceDraftIsDuplicate = Boolean(avoidanceDraftValue) && state.hardAvoidances.some((item) => item.toLowerCase() === avoidanceDraftValue.toLowerCase());
  const hardAvoidancesAtCap = state.hardAvoidances.length >= STORY_FIT_SELECTION_LIMITS.hardAvoidances;
  const canAddAvoidance = Boolean(avoidanceDraftValue) && !avoidanceDraftIsDuplicate && !hardAvoidancesAtCap;
  const toggleLimitedItem = (field: keyof Pick<StoryFitOnboardingState, "preferredStoryTypes" | "emotionalPromises" | "favoriteStoryWorlds" | "storyIngredients" | "characterLensPreferences" | "narrativePressurePreferences" | "episodeEndingShapePreferences" | "protagonistLensPreferences">, value: string, max: number, label: string, options: { label: string }[]) => {
    setState((current) => {
  ...
  const addAvoidance = () => {
    ...
    if (hardAvoidancesAtCap) {
      setLimitMessage(`Hard avoidances are limited to ${STORY_FIT_SELECTION_LIMITS.hardAvoidances} selections.`);
      return;
    }
    const next = addUniquePreferenceItem(state.hardAvoidances, avoidanceDraftValue, STORY_FIT_SELECTION_LIMITS.hardAvoidances);
  ...
  return (
    <section className="mx-auto grid w-full max-w-3xl min-w-0 gap-4 overflow-x-hidden rounded-xl border border-lantern-gold/35 bg-night-ink/95 p-4 pb-24 shadow-soft sm:pb-4">
      ...
      {step === 0 ? <OnboardingChoiceGrid countLabel={`${countVisibleSelections(state.preferredStoryTypes, STORY_FIT_STORY_TYPE_OPTIONS)} / ${STORY_FIT_SELECTION_LIMITS.preferredStoryTypes} selected`} question="Story types / fear flavors" helper={`Choose up to ${STORY_FIT_SELECTION_LIMITS.preferredStoryTypes}.`} options={STORY_FIT_STORY_TYPE_OPTIONS} selected={state.preferredStoryTypes} onSelect={(value) => toggleLimitedItem("preferredStoryTypes", value, STORY_FIT_SELECTION_LIMITS.preferredStoryTypes, "Story types", STORY_FIT_STORY_TYPE_OPTIONS)} /> : null}
      {step === 1 ? <OnboardingChoiceGrid countLabel={`${countVisibleSelections(state.emotionalPromises, STORY_FIT_EMOTIONAL_PROMISE_OPTIONS)} / ${STORY_FIT_SELECTION_LIMITS.emotionalPromises} selected`} question="What should the story give you?" helper={`Choose up to ${STORY_FIT_SELECTION_LIMITS.emotionalPromises}.`} options={STORY_FIT_EMOTIONAL_PROMISE_OPTIONS} selected={state.emotionalPromises} onSelect={(value) => toggleLimitedItem("emotionalPromises", value, STORY_FIT_SELECTION_LIMITS.emotionalPromises, "What should the story give you", STORY_FIT_EMOTIONAL_PROMISE_OPTIONS)} /> : null}
      {step === 2 ? <OnboardingChoiceGrid countLabel={`${countVisibleSelections(state.favoriteStoryWorlds, STORY_FIT_WORLD_OPTIONS)} / ${STORY_FIT_SELECTION_LIMITS.favoriteStoryWorlds} selected`} question="Storyworlds / places I want to live inside" helper={`Choose up to ${STORY_FIT_SELECTION_LIMITS.favoriteStoryWorlds}.`} options={STORY_FIT_WORLD_OPTIONS} selected={state.favoriteStoryWorlds} onSelect={(value) => toggleLimitedItem("favoriteStoryWorlds", value, STORY_FIT_SELECTION_LIMITS.favoriteStoryWorlds, "Storyworlds", STORY_FIT_WORLD_OPTIONS)} /> : null}
      {step === 3 ? <OnboardingChoiceGrid countLabel={`${countVisibleSelections(state.storyIngredients, STORY_FIT_INGREDIENT_OPTIONS)} / ${STORY_FIT_SELECTION_LIMITS.storyIngredients} selected`} question="Story ingredients" helper={`Choose up to ${STORY_FIT_SELECTION_LIMITS.storyIngredients}.`} options={STORY_FIT_INGREDIENT_OPTIONS} selected={state.storyIngredients} onSelect={(value) => toggleLimitedItem("storyIngredients", value, STORY_FIT_SELECTION_LIMITS.storyIngredients, "Story ingredients", STORY_FIT_INGREDIENT_OPTIONS)} /> : null}
...
function SeriesLibraryGroup({ group, onContinueSavedStoryById, onDeleteStory, onOpenSavedStoryById }: { group: LibrarySeriesGroupWithRows; onContinueSavedStoryById: (storyId: string) => void; onDeleteStory: (storyId: string) => void; onOpenSavedStoryById: (storyId: string) => void }) {
...
function MobileLibraryEpisodeCard({ episode, onContinueSavedStoryById, onOpenSavedStoryById }: { episode: SeriesEpisode<LibraryStory> & { row: LibraryStoryRow }; onContinueSavedStoryById: (storyId: string) => void; onOpenSavedStoryById: (storyId: string) => void }) {
  return (
    <article
    ...
    >
      <div className="grid min-w-0 gap-2">
        ...
        <p className="break-words text-xs leading-5 text-paper/50">
          {formatDateTime(episode.story.createdAt)} · {episode.story.wordCount.toLocaleString()} words · {getLibraryStoryCategoryLabel(episode.story)}
        </p>
        <p className="line-clamp-4 break-words text-sm leading-6 text-paper/70">
          {truncateText(episode.story.story, 260)}
        </p>
      </div>
      <div className="grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2" data-mobile-library-card-actions="true">
        <button className="min-h-11 rounded-md bg-lantern-gold px-4 py-3 text-sm font-semibold text-night-ink" onClick={() => onOpenSavedStoryById(episode.story.id)} type="button">Open Story</button>
        <button className="min-h-11 rounded-md border border-lantern-gold/45 bg-paper/10 px-4 py-3 text-sm font-semibold text-lantern-gold" onClick={() => onContinueSavedStoryById(episode.story.id)} type="button">Continue Series</button>
      </div>
    </article>
  );
}
...
function StoryLibraryCard({ badge, onDelete, onOpen, story }: { badge?: string; onDelete?: () => void; onOpen: () => void; story: LibraryStory }) {
  return <article className="min-w-0 rounded-md border border-paper/12 bg-paper/10 p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><h3 className="text-lg font-semibold text-paper">{story.title}</h3><p className="mt-1 text-sm leading-6 text-paper/60">{formatDateTime(story.createdAt)} | {story.wordCount.toLocaleString()} words | {getLibraryStoryCategoryLabel(story)}</p></div>{badge ? <span className="w-fit rounded-md border border-lantern-gold/35 bg-lantern-gold/10 px-2 py-1 text-xs font-semibold text-lantern-gold">{badge}</span> : null}</div><p className="mt-3 text-sm leading-6 text-paper/70">{truncateText(story.story, 220)}</p><div className="mt-4 flex flex-wrap gap-2"><SmallButton onClick={onOpen}>Open</SmallButton>{onDelete ? <SmallButton onClick={onDelete}>Delete</SmallButton> : null}</div></article>;
}
  ...
  return (
    <details className="min-w-0 rounded-md border border-paper/10 bg-paper/5 p-3 text-xs text-paper/65">
      ...
      <div className="mt-3 grid gap-1 sm:grid-cols-2">
        ...
        <p><span className="font-semibold text-paper/80">selectedStoryTypeChipId:</span> {storyTypeSelectionDiagnostics.selectedStoryTypeChipId}</p>
        <p><span className="font-semibold text-paper/80">selectedStoryTypeChipLabel:</span> {storyTypeSelectionDiagnostics.selectedStoryTypeChipLabel}</p>
        <p><span className="font-semibold text-paper/80">storyTypeSelectionMode:</span> {storyTypeSelectionDiagnostics.storyTypeSelectionMode}</p>
        ...
        <p><span className="font-semibold text-paper/80">chipCompatibilityResult:</span> {storyTypeSelectionDiagnostics.chipCompatibilityResult}</p>
        <p><span className="font-semibold text-paper/80">storySeedSource:</span> {storyTypeSelectionDiagnostics.storySeedSource}</p>
        <p><span className="font-semibold text-paper/80">visibleCategoryLabel:</span> {storyTypeSelectionDiagnostics.visibleCategoryLabel}</p>
        ...
        <p><span className="font-semibold text-paper/80">Next Episode clicked:</span> {readerScrollDiagnostics.nextEpisodeClicked}</p>
        <p><span className="font-semibold text-paper/80">Continuation loaded:</span> {readerScrollDiagnostics.continuationLoaded}</p>
        <p><span className="font-semibold text-paper/80">Scroll reset attempted:</span> {readerScrollDiagnostics.scrollResetAttempted}</p>
        <p><span className="font-semibold text-paper/80">Scroll target used:</span> {readerScrollDiagnostics.scrollTargetUsed}</p>
        ...
        {storyResponseEpisodeMomentum ? <section className="mt-3 grid gap-1 rounded-md border border-lantern-gold/20 bg-lantern-gold/5 p-3 sm:col-span-2">
          <p className="font-semibold text-paper/80">Episode Momentum diagnostics</p>
          <p><span className="font-semibold text-paper/80">Active:</span> {storyResponseEpisodeMomentum.active ? "true" : "false"}</p>
          <p><span className="font-semibold text-paper/80">Activation reason:</span> {storyResponseEpisodeMomentum.activation_reason}</p>
          <p><span className="font-semibold text-paper/80">Continue probability:</span> {storyResponseEpisodeMomentum.scores.continue_probability}</p>
          <p><span className="font-semibold text-paper/80">Repair ran:</span> {storyResponseEpisodeMomentum.repair_ran ? "true" : "false"}</p>
          <p><span className="font-semibold text-paper/80">Repair reason:</span> {storyResponseEpisodeMomentum.repair_reason}</p>
          <p><span className="font-semibold text-paper/80">JSON parse failed:</span> {storyResponseEpisodeMomentum.json_parse_failed ? "true" : "false"}</p>
          <p><span className="font-semibold text-paper/80">Fallback used:</span> {storyResponseEpisodeMomentum.fallback_used ? "true" : "false"}</p>
        </section> : null}
        <details className="mt-3 sm:col-span-2">
          <summary className="cursor-pointer font-semibold text-paper/75">Raw JSON</summary>
          <pre className="mt-2 max-h-72 overflow-auto rounded border border-paper/10 bg-night-ink/80 p-3 text-[0.68rem] leading-5 text-paper/70">{JSON.stringify({ readerProfileGenerationSnapshot: lastNewStoryPersonalization.responseSnapshot ?? null, latestStoryResponse: { metadata: { diagnostics: { episodeMomentum: storyResponseEpisodeMomentum ?? null } } } }, null, 2)}</pre>
        </details>
      </div>
    </details>
  );
    ...
    <details className="min-w-0 rounded-md border border-paper/10 bg-paper/5 p-3 text-xs text-paper/65">
      ...
      <div className="mt-3 grid gap-3">
        <div className="grid gap-1 sm:grid-cols-2">
          ...
          <p><span className="font-semibold text-paper/80">Last profile API path/action:</span> {cloudSync.lastApiPath} / {cloudSync.lastApiAction}</p>
          <p><span className="font-semibold text-paper/80">Last sanitized profile API error:</span> {cloudSync.lastSanitizedErrorName}{cloudSync.lastSanitizedErrorCode !== "none" ? ` (${cloudSync.lastSanitizedErrorCode})` : ""}</p>
          ...
          <p><span className="font-semibold text-paper/80">Last cloud save status:</span> {cloudSync.lastSaveOutcome === "saved" ? "success" : cloudSync.lastSaveOutcome}</p>
          <p><span className="font-semibold text-paper/80">Last profile save status for feedback:</span> {canonicalProfile?.signals.feedbackSignalCount ? `profile updated${cloudSync.lastSaveOutcome ? ` / cloud ${cloudSync.lastSaveOutcome === "saved" ? "success" : cloudSync.lastSaveOutcome}` : ""}` : "no feedback saved"}</p>
...
function Status({ children, tone }: { children: ReactNode; tone: "info" | "success" | "error" }) { return <div className={`min-w-0 rounded-md border px-4 py-3 text-sm ${tone === "error" ? "border-ember/40 bg-ember/10 text-ember" : tone === "success" ? "border-green-300/40 bg-green-950/20 text-green-100" : "border-lantern-gold/30 bg-paper/10 text-paper/75"}`}>{children}</div>; }
...
function readStoryFitOnboardingDismissed(): boolean {
  ...
  try { return window.localStorage.getItem(STORY_FIT_ONBOARDING_STORAGE_KEY) === "dismissed"; } catch { return false; }
}

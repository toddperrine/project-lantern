...
import { findLibraryStoryById, findNextSavedEpisodeInSeries, groupStoriesBySeries, type LibrarySeriesGroup, type SeriesEpisode } from "@/lib/series-library";
...
export default function Home() {
  ...
  function handleRestoreStoryById(storyId: string) {
    const story = findLibraryStoryById(savedStories, storyId);
    if (!story) {
...
function SeriesLibraryGroup({ group, onDeleteStory, onOpenCurrentStory, onOpenSavedStoryById }: { group: LibrarySeriesGroupWithRows; onDeleteStory: (storyId: string) => void; onOpenCurrentStory: () => void; onOpenSavedStoryById: (storyId: string) => void }) {
  return <article className="min-w-0 rounded-md border border-paper/12 bg-paper/10 p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><h3 className="text-lg font-semibold text-paper">{group.title}</h3><p className="mt-1 text-sm leading-6 text-paper/60">{group.episodeCount} {group.episodeCount === 1 ? "Episode" : "Episodes"}{group.lastUpdatedAt ? ` | Last updated ${formatDateTime(group.lastUpdatedAt)}` : ""}</p></div><span className="w-fit rounded-md border border-lantern-gold/35 bg-lantern-gold/10 px-2 py-1 text-xs font-semibold text-lantern-gold">Series</span></div><div className="mt-4 grid min-w-0 gap-3">{group.episodes.map((episode) => <StoryLibraryCard badge={episode.row.kind === "current" ? `Recent Story · Episode ${episode.episodeNumber}` : `Episode ${episode.episodeNumber}`} key={`${episode.row.kind}-${episode.storyId}`} onDelete={episode.row.kind === "saved" ? () => onDeleteStory(episode.row.story.id) : undefined} onOpen={episode.row.kind === "current" ? () => { if (episode.storyId === episode.row.story.id) onOpenCurrentStory(); } : () => onOpenSavedStoryById(episode.storyId)} story={episode.row.story} />)}</div></article>;
}
